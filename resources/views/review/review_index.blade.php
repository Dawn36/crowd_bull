                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Review Listing
                            </h1>
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-primary">
                                ...
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here is the list of all of the Review that we have.</div>
                            </div>
                        </div>
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    <button type="button" class="btn btn-primary me-2" onclick="addReview()">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($review); $i++) @php $a=$i; $a++; @endphp

                                        <tr>
                                            <td>{{$a}}</td>
                                            <td>
                                                <a href="{{route('re-view.show',$review[$i]->id)}}" class="fw-bolder text-hover-primary">{{ucwords($review[$i]->review_name)}}</a>
                                            </td>
                                           
                                            <td>
                                                <div class="badge badge-light-success text-success">Active</div>
                                            </td>
                                            <td>{{ucwords($review[$i]->first_name)}} {{ucwords($review[$i]->last_name)}}</td>
                                            <td>{{Date("Y-m-d",strtotime($review[$i]->created_at))}}</td>
                                            <td>
                                                <button onclick="editReview('{{ $review[$i]->id}}')" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2" data-bs-original-title="Edit Review">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('re-view.destroy', $review[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" type="submit" data-bs-toggle="tooltip" data-bs-original-title="Delete Review">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
<script>
    ...
    function editReview(id) {
        url = "{{ route('re-view.edit', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalXlHeading').html('Edit Review');
