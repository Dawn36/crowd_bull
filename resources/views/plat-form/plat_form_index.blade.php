                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            ...
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Platforms Listing
                            </h1>
                            <!--begin::Notice-->
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-primary">
                                ...
                                <!--end::Icon-->
                                <!--begin::Description-->
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here we have a list of all of the Platforms that we have.</div>
                                <!--end::Description-->
                            </div>
                            <!--end::Notice-->
                            <!--end::Heading-->
                        ...
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    ...
                                    <button type="button" class="btn btn-primary me-2" onclick="addPlatForm()" >
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        ...
                                        <tr>
                                            <td><a href="{{route('plat_form.show', $platForm[$i]->id)}}" class="fw-bolder text-gray-800 text-hover-primary mb-1">{{$a}}</a></td>
                                            <td><a href="{{route('plat_form.show', $platForm[$i]->id)}}" class="fw-bolder text-gray-800 text-hover-primary mb-1">{{ucwords($platForm[$i]->plat_form)}}</a></td>
                                            ...
                                            <td>{{$platForm[$i]->avg_ticket_size}}</td>
                                            <td>{{$platForm[$i]->raised_in_past_30_days}}</td>
                                            <td>{{$platForm[$i]->raised_in_past_7_days}}</td>
                                            <td>
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2" onclick="editPlatForm('{{$platForm[$i]->id}}')">
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('plat_form.destroy', $platForm[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
...
<script type="text/javascript">
    function addPlatForm() {
        $.ajax({
            type: 'GET',
            url: "{{ route('plat_form.create') }}",
            success: function(result) {
                $('#myModalLgHeading').html('Add Platform');
    ...
    function editPlatForm(id) {
        url = "{{route('plat_form.edit',':id')}}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Platform');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
